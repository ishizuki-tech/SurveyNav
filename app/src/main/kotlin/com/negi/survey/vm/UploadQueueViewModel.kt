// file: app/src/main/java/com/negi/survey/vm/UploadQueueViewModel.kt
...
class UploadQueueViewModel(app: Application) : AndroidViewModel(app) {
        ...
        wm.getWorkInfosByTagLiveData(GitHubUploadWorker.TAG)
            ...
            .map { workList ->
                workList
                    ...
                    .sortedWith(compareBy<UploadItemUi> { it.priorityRank() }
    ...
    private fun WorkInfo.toUploadItemUi(): UploadItemUi {
        ...
        val url: String? = outputData.getString(GitHubUploadWorker.OUT_FILE_URL)
    ...
    private fun progressName(wi: WorkInfo): String? =
        wi.progress.getString(GitHubUploadWorker.PROGRESS_FILE)?.takeIf { it.isNotBlank() }

    private fun outputName(wi: WorkInfo): String? =
        wi.outputData.getString(GitHubUploadWorker.OUT_FILE_NAME)?.takeIf { it.isNotBlank() }

    private fun inputName(wi: WorkInfo): String? =
        wi.progress.keyValueMap["input_file"] as? String // optional convention
    ...
    companion object {
         ...
         * val vm = viewModel<UploadQueueViewModel>(factory = UploadQueueViewModel.factory(app))
         */
